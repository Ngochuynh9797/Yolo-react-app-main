...
export const cartItemsSlice = createSlice({
    ...
    reducers: {
        ...
        removeItem:(state, action) => {
            ...
            state.value = delItem(state.value, deletedItem)
        }
    }
}
)

const findItem = (arr, item) =>arr.filter((i) =>  i.color === item.color && i.size === item.size && i.slug === item.slug)
const delItem = (arr, item) =>arr.filter((i) =>  i.color !== item.color || i.size !== item.size || i.slug !== item.slug)
